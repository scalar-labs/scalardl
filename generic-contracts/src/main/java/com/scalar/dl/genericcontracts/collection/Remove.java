...
/**
 * `Remove` removes a set of `String` values from the specified collection. See the comment in
 * `Create` for details about the internal data structure of the collection.
 */
public class Remove extends JacksonBasedContract {
  ...
  public JsonNode invoke(
    ...
    if (!isForcedOptionSpecified(arguments)) {
      validateExistence(arguments, collection);
    }
  ...
  private void validateExistence(JsonNode arguments, JsonNode collection) {
    ...
    for (JsonNode objectId : arguments.get(Constants.OBJECT_IDS)) {
      if (!objectIds.contains(objectId.asText())) {
        throw new ContractContextException(Constants.OBJECT_NOT_FOUND_IN_COLLECTION);
      }
    }
  }
