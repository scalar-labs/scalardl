...
public class ChargeAccountCommand extends LedgerClientExecutor implements Callable {
    ...
    @CommandLine.Option(names = {"-id", "--id"}, paramLabel = "ID", description = "account id", required = true)
    String accountId;

    @CommandLine.Option(names = {"-a", "--amount"}, paramLabel = "AMOUNT", description = "amount to charge", required = true)
    int amount;
