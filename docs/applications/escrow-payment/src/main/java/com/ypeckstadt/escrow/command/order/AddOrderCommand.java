...
public class AddOrderCommand extends LedgerClientExecutor implements Callable {
    ...
    @CommandLine.Option(names = {"-id", "--id"}, paramLabel = "ID", description = "order id", required = true)
    String orderId;

    @CommandLine.Option(names = {"-b", "--buyer"}, paramLabel = "BUYER", description = "buyer's account id", required = true)
    String buyerAccountId;

    @CommandLine.Option(names = {"-i", "--item"}, paramLabel = "ITEM", description = "item id", required = true)
    String itemId;
    ...
    public Integer call() throws Exception {
        ...
        try {
            ...
            LOG.info("The order has been added successfully. Waiting for the seller to mark the item as shipped.");
        } catch (Exception e) {
