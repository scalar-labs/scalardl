...
public class ShippedItemsOrderCommand extends LedgerClientExecutor implements Callable {
    ...
    @CommandLine.Option(names = {"-id", "--orderId"}, paramLabel = "ORDER ID", description = "the order id", required = true)
    String orderId;

    @CommandLine.Option(names = {"-s", "--sellerId"}, paramLabel = "SELLER ID", description = "the seller's account id", required = true)
    ...
    public Integer call() throws Exception {
        ...
        try {
            ...
            LOG.info("The order has been updated successfully. Waiting for the buyer to mark the item as received.");
        } catch (Exception e) {
