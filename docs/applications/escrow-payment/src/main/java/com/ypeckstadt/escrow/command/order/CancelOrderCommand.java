...
public class CancelOrderCommand extends LedgerClientExecutor implements Callable {
    ...
    @CommandLine.Option(names = {"-id", "--orderId"}, paramLabel = "ORDER", description = "order id", required = true)
    String orderId;

    @CommandLine.Option(names = {"-a", "--accountId"}, paramLabel = "ACCOUNT", description = "buyer or seller account id", required = true)
    String accountId;
