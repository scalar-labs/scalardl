...
public class AddAccountCommand extends LedgerClientExecutor implements Callable {
    ...
    @CommandLine.Option(names = {"-id", "--id"}, paramLabel = "ACCOUNT ID", description = "the account id", required = true)
    String accountId;

    @CommandLine.Option(names = {"-n", "--name"}, paramLabel = "NAME", description = "the account owner's name", required = true)
    String name;
