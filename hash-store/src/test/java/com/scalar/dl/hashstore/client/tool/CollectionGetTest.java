...
public class CollectionGetTest {
  ...
  void setup() throws Exception {
    commandLine = new CommandLine(new CollectionGet());
  ...
  class call {
    @Nested
    @DisplayName("where collection get succeeds")
    class whereCollectionGetSucceeds {
      @Test
      @DisplayName("returns 0 as exit code with collection found")
      void returns0AsExitCodeWithCollectionFound() throws Exception {
        // Arrange
      ...
      @Test
      @DisplayName("returns 0 as exit code with collection not found")
      void returns0AsExitCodeWithCollectionNotFound() throws Exception {
        // Arrange
        String[] args = new String[] {"--properties=PROPERTIES_FILE", "--collection-id=coll123"};
        CollectionGet command = parseArgs(args);
        HashStoreClientService serviceMock = mock(HashStoreClientService.class);
    ...
    class whereClientExceptionIsThrownByHashStoreClientService {
      ...
      void returns1AsExitCode(@TempDir Path tempDir) throws Exception {
        ...
        assertThat(outputStreamCaptor.toString(UTF_8.name())).contains("Failed to get collection");
      }
    }
  }

  private CollectionGet parseArgs(String[] args) {
    return CommandLineTestUtils.parseArgs(commandLine, CollectionGet.class, args);
  }
}
