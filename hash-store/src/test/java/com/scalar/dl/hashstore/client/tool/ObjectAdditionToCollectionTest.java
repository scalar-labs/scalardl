...
public class ObjectAdditionToCollectionTest {
  private CommandLine commandLine;
  private final ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();

  @BeforeEach
  void setup() throws Exception {
    commandLine = new CommandLine(new ObjectAdditionToCollection());
  ...
  class call {
    @Nested
    @DisplayName("where object addition to collection succeeds")
    class whereObjectAdditionSucceeds {
      @Test
      ...
      void returns0AsExitCodeWithSingleObjectId() throws Exception {
        ...
        verify(serviceMock)
            .addToCollection(collectionIdCaptor.capture(), objectIdsCaptor.capture());
    ...
    class whereClientExceptionIsThrownByClientService {
      ...
      void returns1AsExitCode(@TempDir Path tempDir) throws Exception {
        ...
        doThrow(new ClientException("Failed to add objects", StatusCode.RUNTIME_ERROR))
            .when(serviceMock)
            .addToCollection(anyString(), anyList());
        ...
        assertThat(outputStreamCaptor.toString(UTF_8.name())).contains("Failed to add objects");
      }
    }
  }

  private ObjectAdditionToCollection parseArgs(String[] args) {
    return CommandLineTestUtils.parseArgs(commandLine, ObjectAdditionToCollection.class, args);
  }
}
