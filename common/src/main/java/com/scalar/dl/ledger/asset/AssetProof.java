...
public class AssetProof {
  ...
  public static byte[] serialize(
      String id, int age, String nonce, JsonObject input, byte[] hash, byte[] prevHash) {
    ...
    ByteBuffer buffer =
        ByteBuffer.allocate(
            id.getBytes(StandardCharsets.UTF_8).length
                ...
                + inputString.getBytes(StandardCharsets.UTF_8).length
                + hash.length
                + prevHashLength);
    buffer.put(id.getBytes(StandardCharsets.UTF_8));
    buffer.putInt(age);
    buffer.put(nonce.getBytes(StandardCharsets.UTF_8));
    buffer.put(inputString.getBytes(StandardCharsets.UTF_8));
    buffer.put(hash);
    if (prevHash != null) {
      buffer.put(prevHash);
    }
