...
public class DigitalSignatureValidatorTest {
  ...
  private static final String CERTIFICATE_B =
      "-----BEGIN CERTIFICATE-----\n"
          ...
          + "-----END CERTIFICATE-----";
  private DigitalSignatureSigner signer;
  private DigitalSignatureValidator validator;

  @Before
  public void setUp() {
    signer = new DigitalSignatureSigner(PRIVATE_KEY_A);
    validator = new DigitalSignatureValidator(CERTIFICATE_A);
  }
  ...
  public void validate_SignatureSignedByIncompatiblePrivateKeyGiven_ShouldReturnFalse() {
    // Arrange
    signer = new DigitalSignatureSigner(PRIVATE_KEY_B);
    byte[] bytes = "any_bytes".getBytes(StandardCharsets.UTF_8);
    byte[] signature = signer.sign(bytes);
  ...
  public void validate_ValidatorCreatedWithIncompatibleCertificateGiven_ShouldReturnFalse() {
    ...
    byte[] bytes = "any_bytes".getBytes(StandardCharsets.UTF_8);
    byte[] signature = signer.sign(bytes);
    validator = new DigitalSignatureValidator(CERTIFICATE_B);
