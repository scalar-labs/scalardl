...
public class NamespaceDroppingTest {
  private CommandLine commandLine;

  @BeforeEach
  void setup() {
    commandLine = new CommandLine(new NamespaceDropping());
  }
  ...
  class call {
    ...
    void returns0AsExitCode() throws ClientException {
      ...
      doReturn(true).when(command).confirmDropping();
      ...
      verify(serviceMock).dropNamespace("test_namespace");
    }
  ...
  private NamespaceDropping parseArgs(String[] args) {
    return CommandLineTestUtils.parseArgs(commandLine, NamespaceDropping.class, args);
  }
}
