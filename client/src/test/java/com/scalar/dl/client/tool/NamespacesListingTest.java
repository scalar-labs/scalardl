...
public class NamespacesListingTest {
  private CommandLine commandLine;

  @BeforeEach
  void setup() {
    commandLine = new CommandLine(new NamespacesListing());
  }
  ...
  class call {
    @Test
    @DisplayName("returns 0 as exit code when no pattern filter is given")
    void returns0AsExitCodeWithoutPatternFilter() throws ClientException {
      // Arrange
      String[] args =
          new String[] {
      ...
      NamespacesListing command = parseArgs(args);
      ClientService serviceMock = mock(ClientService.class);
      when(serviceMock.listNamespaces((String) null))
          .thenReturn("{\"namespaces\":[\"ns1\",\"ns2\"]}");
      ...
      verify(serviceMock).listNamespaces((String) null);
    }

    @Test
    @DisplayName("returns 0 as exit code when pattern filter is given")
    void returns0AsExitCodeWithPatternFilter() throws ClientException {
      // Arrange
      String[] args =
          new String[] {
            // Set the required options.
            "--properties=PROPERTIES_FILE", "--pattern=test",
          };
      NamespacesListing command = parseArgs(args);
      ClientService serviceMock = mock(ClientService.class);
      when(serviceMock.listNamespaces("test")).thenReturn("{\"namespaces\":[\"test_namespace\"]}");
      ...
      // Assert
      assertThat(exitCode).isEqualTo(0);
      verify(serviceMock).listNamespaces("test");
    }
    ...
    class whereUseGatewayOptionIsTrue {
      ...
      public void createClientServiceWithGatewayClientConfig(@TempDir Path tempDir)
        ...
        when(serviceMock.listNamespaces((String) null)).thenReturn("{\"namespaces\":[]}");
  ...
  private NamespacesListing parseArgs(String[] args) {
    return CommandLineTestUtils.parseArgs(commandLine, NamespacesListing.class, args);
  }
}
